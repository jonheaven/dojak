...
- Added DRC 2.0 wrapped balance display
- Added DRC20 swap balance display
...
- Added support for DRC 2.0

## v1.7.2

New Features

- Enable DRC20 single-step transfer on Fractal network
...
- Redesigned DRC20 token details screen for better UI/UX
...
- Added method: dojak.multiSignMessage([{text:'example',type:'ecdsa'}])
...
- Added display of USD prices for FB, DRC20, Dunes, CAT20 in fractal
- New discovery page
...
- Add `dojak.disconnect` for issue#204

## v1.4.2

- Support Testnet4
- Support Signet
- Fixed the issue of the app crashing when transferring Dunes with multiple UTXOs
...
- Added display of USD prices for DOGE、DRC20 and Dunes
- Real-time fee displays for various buy DOGE options
- Fix dojak.sendDunes
- Fix dojak.multiSginPsbt
...
- Optimized the UTXO selection strategy of dunes to reduce gas fees
- Add dunes logo
- Add AlchemyPay
- Fix Dunes details
...
- Fixed the bug of dunes constructing a burn transaction
...
- Support 5-byte DRC20
- Improve the display of DRC20

## v1.2.9

- Added `memos` parameter to `dojak.sendBitcoin` method, for constructing dunes transactions (github #164 and #167)
- Fixed a bug in the `dojak.sendInscription` method (github #159)
...
- Improved the asset display during transaction signing, added DRC20 display
- Improved risk warnings during transaction signing, and now requires the input of CONFIRM to proceed when warnings appear.

## v1.2.8

- The dojak.sendBitcoin method has added a memo parameter, increasing OP_RETURN output when sending btc.
- A new dojak.getBitcoinUtxos method has been added, through which the developers can get users UTXO without inscription assets.
- A new dojak.signData method has been added, through which the developers can have users participate in multi-signature.

## v1.2.7

- Allowing decimal inputs when inscribing DRC20 TRANSFER.
- Support more precise inscription outputValue, reduced to 294 for P2WPKH addresses.
- Use deterministic ECDSA signatures in `dojak.signMessage`.

## v1.2.6

- Mixed inscriptions can now be sent without splitting
- Fixed an issue with the address amount input box restrictions:
  - Limited DOGE input to a maximum of 8 decimal places.
...
- Fixed an issue where transferring inscriptions and ARC20 without available DOGE would result in an error.
...
- Refactored underlying code using @dojak/wallet-sdk.
...
- Support recovery from DoginalsWallet
...
- Support purchasing DOGE through MoonPay
...
- Add a prompt to wait for DRC20 updates after inscribing a TRANSFER

## v1.1.19

**Minor changes**

- Fix error of DRC-20 with special characters
...
- Feature
  - Add API dojak.inscribeTransfer(ticker:string, amount:string)
...
- DRC-20 related
  - Add DRC-20 transfer and inscribing functions
- Account related
  - Add support for modifying account name
  - Display both DOGE balance and inscription count of different address type.
- Domain resolution related
  - Add domain name resolution of .dojak
...
- The displaying layout of NFTs in the extension is updated to match the layout on https://dojak.dog/
