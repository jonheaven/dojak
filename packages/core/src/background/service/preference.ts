...
import { AddressFlagType, CHAINS, ChainType, DEFAULT_LOCKTIME_ID, EVENTS } from '@dojak/core/constant';
...
class PreferenceService {
  ...
  getIsFirstOpen = () => {
    if (!this.store) {
      // Return true for first open if store not initialized yet
      return true;
    }
  ...
  getAutoLockTimeId = () => {
    if (!this.store) {
      // Return default value if store not initialized yet
      return 5; // DEFAULT_LOCKTIME_ID
    }
