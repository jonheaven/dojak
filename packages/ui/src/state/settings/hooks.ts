...
import { CHAINS_MAP, ChainType, VERSION } from '@dojak/core/constant';
import { AddressType, NetworkType } from '@dojak/core/types';
import { useWallet } from '@dojak/ui/utils';
import { getAddressType } from '@dojak/ui/utils/bitcoin-utils';
import i18n, { addResourceBundle } from '@dojak/ui/utils/i18n';
...
export function useTxExplorerUrl(txid: string) {
  ...
  if (chain.enum === ChainType.BITCOIN_MAINNET) {
    return `${chain.dojakExplorerUrl}/tx/${txid}`;
...
export function usedojakWebsite() {
  const chainType = useChainType();
  return CHAINS_MAP[chainType].dojakUrl;
}

export function useDoginalsWebsite() {
  const chainType = useChainType();
  return CHAINS_MAP[chainType].doginalsUrl;
}
...
export function useCAT721NFTContentBaseUrl(version: CAT_VERSION) {
  // CAT721 is not supported on Dogecoin networks
  return '';
}

export function useDRC20MarketPlaceWebsite(ticker: string) {
  const chainType = useChainType();
  if (chainType === ChainType.BITCOIN_MAINNET) {
    if (ticker.length == 6) {
      return `${CHAINS_MAP[chainType].dojakUrl}/market/drc20_prog?tick=${ticker}`;
    }
  }
  return `${CHAINS_MAP[chainType].dojakUrl}/market/drc20?tick=${ticker}`;
}

export function useDunesMarketPlaceWebsite(ticker: string) {
  const chainType = useChainType();
