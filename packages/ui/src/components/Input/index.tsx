...
import { ChainType, SAFE_DOMAIN_CONFIRMATION } from '@dojak/core/constant';
import { isDNSName, resolveDNSName, getDNSErrorMessage } from '@dojak/core/lib/dns';
import { getSatsName } from '@dojak/core/lib/satsname-utils';
import { Inscription } from '@dojak/core/types';
import { Icon, Row, Text } from '@dojak/ui/components';
import { useI18n } from '@dojak/ui/hooks/useI18n';
import { getAddressTips, useChain } from '@dojak/ui/state/settings/hooks';
import { colors } from '@dojak/ui/theme/colors';
import { spacing } from '@dojak/ui/theme/spacing';
import { isValidBech32Address, useWallet } from '@dojak/ui/utils';
import { isValidAddress } from '@dojak/ui/utils/bitcoin-utils';
...
function AmountInput(props: InputProps) {
  ...
  const handleInputAmount = (e) => {
    ...
    if (disableDecimal) {
      ...
      if (enableDrc20Decimal) {
        if (/^\d+\.?\d{0,18}$/.test(value) || value === '') {
          setValidAmount(value);
          setInputValue(value);
        }
      } else if (dunesDecimal !== undefined) {
        const regex = new RegExp(`^\\d+\\.?\\d{0,${dunesDecimal}}$`);
