...
import { Column, Content, Header, Icon, Layout, Row, Text, Tooltip } from '@dojak/ui/components';
import { useTools } from '@dojak/ui/components/ActionComponent';
import AssetTag from '@dojak/ui/components/AssetTag';
import { Pagination } from '@dojak/ui/components/Pagination';
import { useI18n } from '@dojak/ui/hooks/useI18n';
import { HistoryDetail } from '@dojak/ui/pages/Wallet/HistoryScreen/HistoryDetail';
import { useAccountAddress } from '@dojak/ui/state/accounts/hooks';
import { fontSizes } from '@dojak/ui/theme/font';
import { satoshisToDOGE, shortAddress, useWallet } from '@dojak/ui/utils';
...
export function AmountItem({ item, inDetail }: { item: ExtraItem; inDetail?: boolean }) {
  ...
  return (
    <Row gap={'sm'} style={{ flexWrap: 'wrap' }} justifyEnd={!inDetail} justifyCenter={inDetail} itemsCenter>
      {
        // item.type !== 'DOGE' && <AssetTag type={item.type} small />
      }
      ...
      <Text
        ...
        size={inDetail ? 'md' : 'xs'}
      />
      {item.type !== 'DOGE' && inDetail && <AssetTag type={item.type} />}
    </Row>
  );
}
              ...
              if (vin.address === address) {
                ...
                if (vin.dunes) {
                  vin.dunes.forEach((r) => {
                    ...
                    assetMap[r.spacedDune].value = assetMap[r.spacedDune].value.minus(r.amount);
                  });
                }
              } else {
              ...
              if (vout.address === address) {
                ...
                if (vout.dunes) {
                  vout.dunes.forEach((r) => {
                    ...
                    assetMap[r.spacedDune].value = assetMap[r.spacedDune].value.plus(r.amount);
                  });
                }
              } else {
