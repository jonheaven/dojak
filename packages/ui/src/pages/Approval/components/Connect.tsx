...
import { Account, WebsiteState } from '@dojak/core/types';
...
export default function Connect({ params: { session } }: Props) {
  ...
  if (checkState === WebsiteState.SCAMMER) {
    return (
      <Layout>
        ...
        <Footer>
          <Row full>
            <Button text={t('reject_blocked_by_dojak_wallet')} preset="danger" onClick={handleCancel} full />
          </Row>
        </Footer>
      </Layout>
    );
  ...
  return (
    <Layout>
      ...
      <Content>
        <Column>
          <Text text={t('connect_with_dojak_wallet')} preset="title-bold" textCenter mt="lg" />
