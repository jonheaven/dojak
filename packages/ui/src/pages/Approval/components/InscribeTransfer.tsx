...
import { InscribeOrder, RawTxInfo, TokenBalance, TokenInfo, TxType } from '@dojak/core/types';
import {
  Button,
  Card,
  Column,
  ...
  Text,
  Tooltip
} from '@dojak/ui/components';
import { useTools } from '@dojak/ui/components/ActionComponent';
import { Loading } from '@dojak/ui/components/ActionComponent/Loading';
...
import { OutputValueBar } from '@dojak/ui/components/OutputValueBar';
import { RBFBar } from '@dojak/ui/components/RBFBar';
...
import { useNetworkType, useDOGEUnit } from '@dojak/ui/state/settings/hooks';
import {
  useFetchUtxosCallback,
  usePrepareSendBypassHeadOffsetsCallback,
  usePushBitcoinTxCallback
} from '@dojak/ui/state/transactions/hooks';
import { fontSizes } from '@dojak/ui/theme/font';
import { spacing } from '@dojak/ui/theme/spacing';
import { amountToSatoshis, satoshisToAmount, useApproval, useLocationState, useWallet } from '@dojak/ui/utils';
import { getAddressUtxoDust } from '@dojak/ui/utils/bitcoin-utils';
...
function InscribeTransferStep({ contextData, updateContextData }: StepProps) {
  ...
  useEffect(() => {
    fetchUtxos();
    wallet
      .getDRC20Summary(account.address, contextData.ticker)
      .then((v) => {
  ...
  const onClickInscribe = async () => {
    try {
      ...
      const order = await wallet.inscribeDRC20Transfer(
        account.address,
        contextData.ticker,
        amount,
        feeRate,
          ...
          <Column gap="lg" full>
            ...
            <Column>
              <Row justifyBetween itemsCenter>
                ...
                <TickUsdWithoutPrice tick={contextData.ticker} balance={inputAmount} type={TokenType.DRC20} />
                {tokenBalance ? (
                  <Column>
          ...
          <Column gap="lg" full>
            ...
            <Column justifyCenter style={{ height: 250 }}>
              <Row itemsCenter justifyCenter>
                ...
                <DRC20Ticker tick={tokenBalance.ticker} displayName={tokenBalance.displayName} preset="lg" />
              </Row>
              <Row itemsCenter justifyCenter>
                <TickUsdWithoutPrice tick={tokenBalance.ticker} balance={amount + ''} type={TokenType.DRC20} />
              </Row>
              <Column mt="xxl">
                <Text text={t('preview')} preset="sub-bold" />
                <Card preset="style2">
                  <Text
                    text={`{"p":"drc-20","op":"transfer","tick":"${tokenBalance.ticker}","amt":"${amount}"}`}
                    size="xs"
                    wrap
                  />
                </Card>
...
function InscribeResultStep({
  ...
  const onClickConfirm = () => {
    tools.showLoading(true);
    wallet
      .getDRC20Summary(currentAccount.address, tokenBalance.ticker)
      .then((v) => {
      ...
      <Content style={{ gap: spacing.small }}>
        <Column justifyCenter mt="xxl" gap="xl">
          ...
          <Column justifyCenter itemsCenter style={{ width: '100%', alignItems: 'center' }}>
            ...
            <Column mt="lg">
              <Text text={t('the_transferable_and_available_balance_of_drc20_wi')} textCenter />
            </Column>
          </Column>
        </Column>
      </Content>
