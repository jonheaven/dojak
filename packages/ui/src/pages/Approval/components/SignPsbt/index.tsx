...
import { Button, Column, Content, Footer, Header, Icon, Layout, Row, Text } from '@dojak/ui/components';
import { useTools } from '@dojak/ui/components/ActionComponent';
import ColdWalletSignPsbt from '@dojak/ui/components/ColdWallet/SignPsbt';
import { ContractPopover } from '@dojak/ui/components/ContractPopover';
import LoadingPage from '@dojak/ui/components/LoadingPage';
import { PhishingDetection } from '@dojak/ui/components/PhishingDetection';
import { SignPsbtWithRisksPopover } from '@dojak/ui/components/SignPsbtWithRisksPopover';
import WebsiteBar from '@dojak/ui/components/WebsiteBar';
import { useI18n } from '@dojak/ui/hooks/useI18n';
import KeystoneSignScreen from '@dojak/ui/pages/Wallet/KeystoneSignScreen';
import { useAccountAddress, useCurrentAccount } from '@dojak/ui/state/accounts/hooks';
import { useDOGEUnit } from '@dojak/ui/state/settings/hooks';
import {
...
import { satoshisToAmount, useApproval, useWallet } from '@dojak/ui/utils';
...
export default function SignPsbt({
  ...
  // get price data
  const { drc20PriceMap, dunesPriceMap } = usePriceFetcher(txInfo, wallet, tools);
  ...
  const detailsComponent = useMemo(
    () => (
      <SignTxDetails
        ...
        drc20PriceMap={drc20PriceMap}
      />
    ),
    [txInfo, drc20PriceMap, dunesPriceMap, type, rawTxInfo]
  );
