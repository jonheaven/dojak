...
import { Column, Input, Row, Spin, Text } from '@dojak/ui/components';
import { useI18n } from '@dojak/ui/hooks/useI18n';
import { useChain } from '@dojak/ui/state/settings/hooks';
...
export function SearchBar() {
  ...
  const { searchContent, onSearch } = useMemo(() => {
    if (value) {
      if (isAddress(value)) {
        const onSearch = () => {
          window.open(chain.dojakExplorerUrl + '/address/' + value);
        };
      ...
      if (isTxid(value)) {
        const onSearch = () => {
          window.open(chain.dojakExplorerUrl + '/tx/' + value);
        };
        return {
          searchContent: (
            <Row py={'lg'} px={'lg'} clickable onClick={onSearch}>
      ...
      if (isBlock(value)) {
        const onSearch = () => {
          window.open(chain.dojakExplorerUrl + '/block/' + value);
        };
        return {
          searchContent: (
            <Row py={'lg'} px={'lg'} clickable onClick={onSearch}>
      ...
      const onSearch = () => {
        window.open(chain.dojakUrl + '/search2?type=text&q=' + value);
      };
    ...
    function gotoExplorer() {
      window.open(chain.dojakExplorerUrl);
    }
